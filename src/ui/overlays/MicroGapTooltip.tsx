...
import { MIN_GAP_MM, TOOLTIP_GAP_MAX_MM, EPS_NORM_MM, EPS_UI_MM, uiLE, uiNEAR } from '@/constants/validation';
...
function scenePointToClient(p: { x: number; y: number }) {
  ...
  const domPt = (window as any).DOMPoint ? new DOMPoint(p.x, p.y) :
    (svg as any).createSVGPoint?.call(svg, p.x, p.y) ?? { x: p.x, y: p.y, matrixTransform: () => ({ x: p.x, y: p.y }) };
...
export default function MicroGapTooltip() {
  ...
  const [anchorPx, setAnchorPx] = useState<{left:number; top:number}>({ left: -9999, top: -9999 });
  ...
  }, [scene.revision, subjectBBox?.x, subjectBBox?.y, subjectBBox?.w, subjectBBox?.h, ui.selectedId, ui.selectedIds, canUseOverride, ui.transientBBox]);
  ...
  if (gapMm != null && gapMm < MIN_GAP_MM) {
    ...
    label = '1.00 mm';
  } else if (prev === 'Bord à bord' && gapMm != null && uiLE(MIN_GAP_MM, gapMm) && uiLE(gapMm, MIN_GAP_MM + EPS_NORM_MM)) {
  ...
  const anchorScene = useMemo(() => {
    ...
    return side === 'left'   ? { x: sb.x,           y: sb.y + sb.h / 2 } :
           side === 'right'  ? { x: sb.x + sb.w,    y: sb.y + sb.h / 2 } :
           side === 'top'    ? { x: sb.x + sb.w/2,  y: sb.y } :
  ...
  }, [scene.revision, subjectBBox?.x, subjectBBox?.y, subjectBBox?.w, subjectBBox?.h, side, anchorScene]);
  ...
  return (
    <div className="micro-gap-tooltip" style={style} data-testid="micro-gap-tooltip" aria-hidden="true">
      {label}
    </div>
  );
}
