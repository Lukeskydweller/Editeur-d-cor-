...
describe('snap group excludes group members (GLOBAL_IDX)', () => {
  ...
  it('excludes all group members from snap candidates (GLOBAL_IDX)', async () => {
    ...
    const id2 = await store.insertRect({ w: 50, h: 50, x: 51, y: 0 }); // 1mm spacing from id1
    ...
    // Group bbox: [0..101] × [0..50] (includes both pieces with 1mm spacing)
    const groupBBox = { x: 0, y: 0, w: 101, h: 50 };
    ...
    const groupBBoxNearId3 = { x: 144, y: 0, w: 101, h: 50 }; // Right edge at 245, within 5mm of id3.left (200)
